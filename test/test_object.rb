...
class TC_Object < Test::Unit::TestCase
	...
	assert(object.respond_to?(:block=))
	assert(Hash === object.as_hash)
	assert(Array === object.block)

	new_hash = Hash.new

	assert_same(object.as_hash, object.as_hash)
	object.as_hash = new_hash
	assert_same(new_hash, object.as_hash)

	assert_same(object.block, object.block)
	...
	klass = Class.new do
	    ...
	    class_attribute :as_hash => Hash.new # do NOT use :as_hash here as it would override #as_hash which is a quite useful method ...
